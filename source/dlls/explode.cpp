...
void ExplosionCreate( const Vector &center, const Vector &angles, edict_t *pOwner, int magnitude, BOOL doDamage, int inExplosionTeam)
{
	KeyValueData	kvd;
	char			buf[128];

	CBaseEntity *pExplosion = CBaseEntity::Create( "env_explosion", center, angles, pOwner );
	CEnvExplosion* theExplosion = dynamic_cast<CEnvExplosion*>(pExplosion);
	ASSERT(theExplosion);

	pExplosion->pev->team = inExplosionTeam;
